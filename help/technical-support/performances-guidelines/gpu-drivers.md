...
breadcrumb-title: ''
description: 렌더링 성능과 안정성을 최적화하기 위한 Substance 3D Painter의 GPU VRAM 및 드라이버 요구 사항에 대해 알아보십시오.
...
일반적으로 VRAM Painter에서 액세스 권한을 많이 가질수록 실행 속도는 빨라지므로 Painter과 동시에 실행되는 다른 응용 프로그램에서 사용하는 VRAM의 양을 최소화하려고 합니다.

## GPU VRAM 양 및 대역폭

Substance 3D Painter은 대부분의 계산을 GPU에 의존합니다. [시스템 요구 사항](../../getting-started/system-requirements.md)을 준수하는 GPU가 중요한 이유가 여기에 있습니다.

Painter은 최종 텍스처를 만들기 위한 혼합 작업과 같이 계산을 수행하기 위해 텍스처를 GPU 메모리(VRAM)로 전송하여 작동합니다. 그러나 VRAM이 가득 차기 시작하면 사용하지 않은 텍스처가 컴퓨터의 RAM으로 다시 전송되어 VRAM 공간을 확보합니다. Substance 3D Painter은 작업 시 GB의 데이터를 쓰고 읽습니다. 즉, 전송을 수행할 때 VRAM의 용량(양)과 대역폭 속도가 모두 중요합니다. [MSI AfterBurner](https://www.msi.com/page/afterburner)와 같은 도구를 사용하여 이 동작을 모니터링할 수 있습니다.
