...
일부 구성의 굽는 동안 Substance 3D Painter이 충돌할 수 있습니다. 이 페이지에서는 알려진 문제 및 이러한 문제를 완화하는 방법에 대한 목록을 다시 그룹화합니다.

## 베이킹 미리 보기와 충돌

기본적으로 Substance 3D Painter은 뷰포트에 텍스처 제빵의 진행 중 상태로 표시됩니다. 일부 컴퓨터에서는 이 기능이 불안정하게 될 수 있습니다.
...
드라이버가 불안정한 일부 GPU에서는 GPU 광선 추적 기능으로 인해 베이킹 프로세스가 충돌할 수 있습니다.
...
Ryzen CPU로 실행되는 일부 컴퓨터 구성에서 베이킹 프로세스 중에 응용 프로그램이 충돌할 수 있습니다. 일반적으로 BIOS 업데이트로 문제가 해결됩니다.
...
베이킹 시 기본적으로 하이 폴리 메시는 **\*.assbin** 파일로 사전 처리되어 나중에 다시 베이킹 속도를 높일 수 있습니다. 드문 경우이지만 이러한 파일이 다른 버전으로 생성된 경우 애플리케이션이 충돌할 수 있습니다. 이러한 파일을 삭제하면 문제가 해결됩니다. 그러면 다시 생성됩니다.
