...
breadcrumb-title: ''
description: Substance 3D Painter에서 텍스처 확장 및 패딩을 사용하여 내보낸 텍스처의 가장자리 아티팩트를 방지하는 방법에 대해 알아봅니다.
...
**패딩**(**확장**&#x200B;이라고도 함)은 텍스처를 생성한 후에 발생하는 프로세스입니다. 이는 UV 섬의 테두리를 확장하여 빈 영역을 유사한 픽셀로 채우는 데 사용됩니다.

좋은 품질의 패딩을 생성하는 것은 나중에 게임 엔진이나 오프라인 렌더러가 [밉맵](../../../getting-started/glossary.md)을 잘 생성하도록 하는 데 중요합니다.\
Substance 3D Painter에서 무한 패딩을 생성할 수 있습니다. 즉, 픽셀이 다른 UV 섬 또는 텍스처 테두리에 도달할 때까지 확장됩니다.
...
3D 컴퓨터 그래픽에서 **밉맵**&#x200B;은 미리 계산되어 최적화된 텍스처 시퀀스입니다. 각 텍스처 시퀀스는 동일한 이미지의 점차 낮은 해상도로 표현됩니다. 이 효과는 렌더링 속도를 높이고 앨리어싱 아티팩트를 줄이기 위한 것입니다. 카메라에 가까운 객체에는 고해상도 밉맵 이미지가 사용됩니다. 저해상도 이미지는 개체가 더 멀리 나타날 때 사용합니다. 이렇게 하면 렌더링하는 것이 효율적이고 원본 텍스처의 픽셀을 모두 읽을 수 있습니다. 밉맵(각 레벨)은 텍스처 자체에 포함됩니다(파일 형식에서 지원하는 경우).

패딩은 텍스처 해상도를 낮출 때 잘못된 색상이 메쉬의 UV 내에서 번지지 않도록 하기 때문에 밉맵에 매우 중요합니다.
