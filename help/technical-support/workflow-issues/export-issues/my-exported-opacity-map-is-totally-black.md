...
새 프로젝트를 만들 때 기본 색상은 텍스처가 아닌 셰이더에서 옵니다. 따라서 칠하지 않은 모든 부품을 내보내면 해당 부품에 데이터가 없기 때문에 알파 값이 0으로 설정된 검정색이 됩니다.

이 문제를 해결하는 가장 쉬운 방법은 레이어 스택 맨 아래에 채우기 레이어를 추가하는 것입니다. 이 경우 셰이더의 기본 색상과 동일한 기본 색상으로 모든 UV를 채웁니다.
