...
* **새 프로젝트 설정**\
  새 프로젝트를 만들 때 이제 새로 추가된 **Adobe Color Engine**(ACE)을 사용하여 색상 관리 엔진을 지정할 수 있습니다.

  ![](../assets/cm-icc-ace-settings.png){width="400px"}

  ACE에는 다음과 같은 작업 색상 공간이 제공됩니다.
...
* **표준 구부리기 제빵사**&#x200B;표준 구부리기 제빵사는 오클루전 방향(노멀 맵과 유사한 벡터)을 제빵할 수 있습니다. 이 텍스처는 [셰이더 설정](../interface/shader-settings/shader-settings.md) 창에서 **구부러진 표준** 설정을 활성화하여 뷰포트의 음영을 개선하는 데 사용할 수 있습니다. 표준 구부리기를 사용하면 실시간 뷰포트 음영의 정확도가 크게 향상됩니다.\
...
* **개선된 재질 피커 동작**\
  이제 도구 도구 모음(키보드 단축키 P)의 재료 선택기는 속성 창 안의 채널 선택을 고려합니다. 채널 자체에서 더 이상 활성화되지 않습니다.
