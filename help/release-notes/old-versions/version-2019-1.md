...
>[!NOTE]
>
> &quot;**정규로 Height**&quot;이라는 새 필터가 셸프에 추가되어 최종 노멀 맵을 가져오는 데 사용할 수 있습니다(Substance Painter에 의한 기본 변환이 충분하지 않은 경우).
...
레이어를 마우스 오른쪽 버튼으로 클릭하고 단축키 &quot;**Height 조합을 사용한 마스크 추가**&quot;를 선택하여 레이어에 이 새 마스크를 빠르게 추가할 수 있습니다. 또한 이 단축키는 Height 채널 혼합 모드를 기본 &quot;선형 닷지(추가)&quot; 대신 &quot;표준&quot;으로 전환합니다.\
...
방사형 대칭을 처리할 수 있도록 대칭 도구의 기능을 확장했습니다. 이제 대칭 설정 메뉴에 이를 활성화하는 새로운 모드가 있습니다(컨텍스트 도구 모음에서 사용 가능).
...
또한 페인트 를 시작하기 전에 설정을 보다 쉽게 조정할 수 있도록 약간의 미리 보기를 추가했습니다.
...
* **평면 프로젝션을 위한 표면 도구**\
  평면 프로젝션을 쉽게 조작할 수 있도록 &quot;**Shift+W**&quot; 단축키로 액세스할 수 있는 **표면 도구**&#x200B;라고 하는 3D 조작기에 대한 새 컨트롤도 추가했습니다. 또한 컨텍스트 도구 모음에서 액세스할 수도 있습니다. 이 새로운 모드는 평면 투영에서만 사용할 수 있습니다.

  ![](../../assets/surface-tool-toolbar.png)

  ![](../../assets/surface-tool-optim.gif)
* **평면 투영 컬링/페이딩**\
  여러 설정을 사용하여 평면 투영을 연속적이거나 한정적으로 만들 수 있습니다. 컬링 설정을 활성화하면 조작기 주위의 doted 상자가 투영에 대한 경계 상자를 나타내고 중간 줄이 투영이 시작되는 지점입니다. 투영의 크기를 조절하면 투영의 이동 거리 및 페이드 시작 시간을 제어할 수 있습니다.
...
* **회전 조작기 모든 축 모드 개선** 명시적인 구를 그리는 대신, 아래의 텍스처링을 숨기지 않도록 구를 숨깁니다. 축 사이를 클릭하면 모든 축을 한 번에 회전할 수 있는 구가 선택됩니다.\
...
* **텍스처 집합 목록에서 빠른 표시/숨기기**\
  이제 레이어 스택과 같이 텍스처 세트를 클릭하고 드래그하여 숨기거나 표시할 수 있습니다.
* **레이어 스택의 UI 개선**\
...
* **더 나은 히스토그램**\
  히스토그램 생성기를 재작업했습니다. 이제 막대 그래프가 더 정확한 정보를 표시하고 레이어 스택 변경 후 올바르게 업데이트됩니다.\
...
* **7개의 새 생성기**\
  ...
  * **월드 스페이스 표준**: 월드 스페이스 표준 메시 맵에 대한 컨트롤을 제공하는 마스크 생성기. 기반으로 합니다.
  * **위치** : 위치 메시 맵을 제어하는 마스크 생성기. 기반으로 합니다.
  * **곡률** : 곡률 메시 맵을 제어하는 마스크 생성기. 기반으로 합니다.
  * **자동 스티처** : UV 테두리, 메시 곡률 또는 사용자 지정 마스크 입력 주위에 스티치를 만드는 마스크 생성기.
