---
helpx_url: "https://helpx.adobe.com/kr/substance-3d-painter/release-notes/old-versions/version-7-3.html"
breadcrumb-title: ''
...
이 릴리스에서는 칠 레이어 및 칠 효과에 대해 새로운 3D 뒤틀기 투영을 도입합니다. 이 투영을 통해 변형 그리드와 제어 가능한 점을 사용하여 텍스처 또는 이미지를 왜곡할 수 있습니다.

* **드래그 앤 드롭을 통한 빠른 설정**&#x200B;에셋 라이브러리에서 재질, 알파, 텍스처 또는 프로시저를 선택하고 메쉬의 원하는 부분(재질에 필요한 단축키 **ALT**)으로 드래그 앤 드롭합니다. 에셋이 재질이 아닌 경우 할당할 채널을 묻는 팝업이 표시됩니다.\
...
* **표면 도구를 사용한 자동 배치**&#x200B;새 뒤틀기 레이어가 만들어지면 표면 도구가 자동으로 선택됩니다. 그러면 이미지를 이동해서 메시 표면에 항상 유지할 수 있습니다. 그러나 언제든지 다른 조작기로 전환하고 변환(단축키 **W**), 회전(단축키 **E**) 또는 비율(단축키 **R**)을 조정할 수 있습니다. 표면 도구로 돌아가려면 단축키 **SHIFT + W**&#x200B;를 사용하십시오. *정점 편집* 모드로 전환할 때 [표면] 도구도 기본 선택이며 이 도구는 정점의 이동을 메시 표면에 스냅합니다. 그러나 **CTRL을 유지**&#x200B;하여 서피스 도구를 일시적으로 빠르게 재정의할 수 있습니다. 이렇게 하면 선택한 점을 서피스뿐만 아니라 임의의 방향으로 이동할 수 있습니다.

* **쉽게 편집할 수 있는 뒤틀기 격자**&#x200B;이미지의 전체 배치가 완료되면 뒤틀기 격자 자체를 편집하여 정확도와 유연성을 높일 수도 있습니다. 격자 편집 모드로 전환하려면 새로 추가된 뒤틀기 메뉴를 사용하거나 **SHIFT + V** 단축키를 사용할 수 있습니다. 그러면 격자의 기존 정점을 편집할 수 있습니다.\
...
이 릴리스에는 칠 레이어와 칠 효과에 대한 원통형 투영 방법이 추가되었습니다. 새로운 투영을 통해 기둥, 기둥 또는 캐릭터의 팔과 같은 기타 오가닉 모양과 같은 개체 주위에 이미지 또는 텍스처를 맞출 수 있습니다.
...
* **색상 피커 창이 열려 있음**\
  이제 색상 피커 창을 다른 화면에서와 같이 옮겨 아무 곳에나 배치할 수 있으며 컨텍스트 스위치가 없는 한 열린 상태로 유지됩니다. 즉, 텍스처를 손으로 칠하는 동안 레이어 페인팅 간을 전환할 때 색상 피커 창을 열어 쉽게 액세스할 수 있습니다.
