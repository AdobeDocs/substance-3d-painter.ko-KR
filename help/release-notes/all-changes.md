...
* [베이킹] 비동기 저장을 통해 UV 타일 프로젝트의 베이킹 시간 개선
* [Shaders] Vulkan 마이그레이션 후 셰이더 API 변경 로그 변경 언급
* OpenEXR을 버전 3.4.4로 업데이트

<b>고정</b>:

* [충돌] Nvidia GTX 10xx 시리즈에서 시작하는 동안 충돌이 발생합니다
...
* [베이킹] ID 맵이 [높음] 모드에서 FBX의 꼭지점 색상을 베이킹하지 않음
...
* [VFX Platform 2025] 색상 관리 설정에 ACES 2.0 구성 추가
* [VFX Platform 2025] OCIO를 버전 2.4.2로 업데이트
...
* [충돌] 이미지 입력 삭제
* 레이어 스택 버튼을 통해 스마트 매트를 추가할 수 없음
...
* [자동 풀기] 텍셀 밀도를 통해 UV 섬을 UDIM으로 더 잘 분할할 수 있습니다.
...
* [충돌] 드문 경우지만 Illustrator 서버가 충돌합니다
...
* [Python][USD] USD 형식의 프로젝트 구성 매개 변수를 표시합니다.
* [Python][USD] USD 형식의 프로젝트 생성 매개 변수를 표시합니다.
* [내보내기][USD] 내보낸 USD 파일 내에 프로젝트 경로 정보 추가
* [GLTF] GLTF 파일을 다시 로드할 때 라이브러리의 텍스처를 업데이트합니다
...
* [엔진] 경우에 따라 텍스처/투영이 빈 뒤틀기 모드입니다.
* [Iray] 렌더링을 저장할 때 이터레이션이 0으로 재설정됩니다.
* [Log] [파일] > [새로 만들기]를 수행할 때 USD 오류 메시지가 표시됨
...
* [도구] 투영 미리 보기가 사양/광택 작업 과정으로 확산으로 전환되지 않음
...
* [Shelf] 새로운 내보내기 사전 설정 : Vray UDIM, Arnold UDIM, Metal/Rough의 Spec/Gloss
...
* [Shelf] Specular/광택 템플릿에서 잘못된 셰이더를 사용합니다.
* [내보내기] 기본 내보내기 사전 설정이 일부 맵(예: AO)을 내보내지 않음
* [뷰포트] 2D 보기에서 UV 바깥쪽을 클릭하면 피벗 포인트가 업데이트되지 않음
...
* substance share으로 두 번째 내보내기 후 충돌 발생
...
* [Shelf] 스마트 재료 및 생성기를 위한 새로운 미리보기 구
* [Shelf] Specular 광택 셰이더 추가
* [선반] 새로운 단단한 표면 모양
* [Shelf] 새로운 Alpha 텍스처 및 모양
* [Shelf] 새로운 피부 텍스처
* [Shelf] 새로운 스캔 기반 재료 및 스마트 재료
* [Shelf] 구형 소재의 새로운 스마트 소재 및 사양/광택 지원
...
* [뷰포트] 기본 일반 텍스처의 정밀 문제로 인해 잘못된 반사가 발생함
* [뷰포트] 기본적으로 비네팅 활성화됨
* [뷰포트] 환경 맵 테두리에 가공물이 나타납니다(Nvidia GPU)
* [뷰포트] 투영/스텐실 모드의 축소판을 불러오기가 매우 깁니다.
* [베이커] 32비트 대신 16비트 정수로 구운 텍스처를 저장합니다
* [레이어] 오래된 물질이 스택에 잘못 표시됨
* 일부 채널의 기본 색상 및 비트 깊이가 올바르지 않습니다(예: Specular, 광택).
...
* [도구] Windows의 브러시 알파에서 비트맵을 사용할 때 충돌이 발생합니다
...
* [Shelf] 스마트 재질은 로드 속도가 매우 느립니다.
* [Windows] 라이선스 서비스 설치 수정
* [채널] 전송 맵은 기본적으로 G8로 만들어집니다
...
* [재질] 확산 채널은 재질이 있는 BaseColor 매핑을 재정의합니다.
* [3D 보기] 특정 환경 맵과 함께 잘못된 확산 조명
* [도구] 브러시를 특정 각도로 회전할 수 없습니다
* 텍스트 필드에서 입력하는 동안 뷰포트가 켜져 있으면 포커스가 생김
* 현재 버전의 선반에 대해 너무 최근 사전 설정으로 충돌
* 메시를 교체한 후 충돌 발생
...
* [Mac] Mac OS X Yosemite(10.10)와 충돌
...
* [베이커][3D 뷰] 접선/이항식이 없는 경우 Mikkt 접선 공간 계산을 사용합니다.
...
* Height 맵 페인팅은 음수 값을 페인팅하지 않습니다.
* 재질 선택기 표시에서는 샘플링된 표준 맵을 고려하지 않아야 합니다
* 입자 결정론 고장
* 2D 보기의 스텐실 매트릭스
* Obj 파일의 Ngons
* 다양한 충돌
