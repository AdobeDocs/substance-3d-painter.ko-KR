...
<b>Substance 3D Painter 10.1</b>에는 새로운 강력한 필터, 개선된 USD 기능 및 업데이트된 VFX 플랫폼 및 Linux 지원이 추가되었습니다.
...
* <b>변형 및 미러링 필터 개선</b>\
  변형 필터가 비균일 크기 조절, 수평 또는 수직으로 뒤집기 등을 지원하도록 업데이트되어 매개 변수 사용이 더욱 간편해졌습니다. 미러 필터를 보다 간단한 매개 변수로 새로 고쳤습니다.
...
* <b>USD로 Adobe 표준 재질로 텍스처 내보내기\
  </b>Painter에서 USD 파일로 텍스처를 내보낼 때 Adobe 표준 재질 속성이 함께 제공됩니다. 이렇게 하면 이러한 USD 파일을 해당 속성을 지원하는 애플리케이션에서도 사용할 수 있습니다.
* <b>USD 파일에서 텍스처 가져오기</b>\
  이제 USD 파일을 가져오면 파일이 만드는 프로젝트의 텍스처도 가져와서 응용 프로그램 간에 쉽게 주고받을 수 있습니다. USD 파일에서 Adobe Standard Material을 사용하는 경우 이 역시 셰이더 설정을 구성하여 뷰포트의 결과가 다른 소스 응용 프로그램과 일치하게 됩니다.
* <b>Gltf 변경 내용\
  </b>USD 업데이트 이후에 GLTF 형식의 동작을 일부 변경하여 패리티를 유지해야 했습니다. 이제 gltf 파일을 가져올 때 Painter에서는 표준 맵이 OpenGL 형식으로 된 것으로 가정합니다.\
  일부 gltf 파일에는 DirectX 형식이 대신 사용될 수도 있습니다. 따라서 새 프로젝트 창에서 해당 설정을 고려할 수 있도록 새 설정이 추가되었습니다(표준 포맷은 레이어 스택에서 오버라이드될 수도 있음).
