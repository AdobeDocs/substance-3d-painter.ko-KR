---
helpx_url: "https://helpx.adobe.com/kr/substance-3d-painter/release-notes/version-11-1.html"
breadcrumb-title: ''
...
>[!NOTE]
>
> 이 버전의 Painter은 그래픽 API를 OpenGL에서 Vulkan으로 전환합니다. 이 변경은 특히 GPU 기반 광선 추적을 사용한 굽기를 위해 응용 프로그램에서 지원하는 GPU에 영향을 줄 수 있습니다.
> 
...
<b>리본 경로</b>은(는) 패스 도구 모음에 새로 추가된 도구입니다. 리본은 날카로운 모퉁이를 위한 옵션뿐만 아니라 시작 및 끝에 대한 추가 제어와 함께, 어떤 컷 없이 패스를 따라 텍스처를 반복할 것입니다.
...
* <b>리본은 모든 종류의 표면에 걸쳐 작동하는 연속 경로입니다.</b>\
  ItRibbon은 경로를 따라 늘리다를 반복하거나 반복할 수 있는 도구입니다. 메쉬 부품이 연결되지 않은 경우에도 모든 종류의 서피스와 형상에 걸쳐 작동합니다.
...
* <b>타일링 제어</b>\
  이미지는 자동 또는 수동으로 리본 경로를 따라 반복되거나 반복될 수 있습니다.
...
* <b>대칭과 호환</b>\
  다른 유형의 도구와 마찬가지로 리본 경로는 대칭 기능과도 호환됩니다.
...
* <b>새 콘텐츠</b>\
  ...
  * <b>텍스트</b>: 다른 정렬 모드와 늘리다를 사용하여 리본으로 경로를 따라 텍스트를 설정하는 빠른 사전 설정입니다.
...
칠 레이어 및 효과는 이제 3D 투영 모드를 통해 대칭을 지원합니다. 이 기능은 컨텍스트 도구 모음의 대칭 메뉴나 <b>속성</b> 창의 새로 추가된 대칭 섹션을 통해 활성화할 수 있습니다.

* <b>채우기 레이어 </b>의 대칭\
  이제 채우기 효과 및 레이어에서 3D 기반 투영 모드를 사용할 때 대칭을 활성화할 수 있습니다. 거울 및 방사형 대칭을 모두 사용할 수 있습니다.

  ![](../assets/symmetry_fill.jpg)
* <b>상황에 맞는 대칭 모음 또는 속성 창을 통해 속성을 사용합니다</b>\
...
* <b>텍스트 및 로고에 대한 입력 리소스 뒤집기</b>\
  칠 레이어 및 효과 대칭은 입력 이미지 또는 X/Y 축을 뒤집을 수 있는 새로운 옵션의 장점도 가지고 있습니다. 이를 통해 예를 들어 텍스트를 미러링할 수 있지만 여전히 양쪽에서 읽을 수 있게 합니다.

  ![](../assets/mirror_symmetry_flip.gif)
* <b>개선된 대칭 설정 인터페이스</b>\
  대칭 설정 인터페이스를 읽기 쉽고 빠르게 사용할 수 있도록 재작업했습니다. 축 슬라이더에는 각각 고유한 선이 있으므로 보다 정확하게 할 수 있습니다. 방사형 디스플레이는 또한 더 적은 공간을 차지하도록 소형화되었다.
...
* <b>Windows 및 Linux에서 OpenGL 대신 Vulkan 그래픽 API가 사용되었습니다</b>\
  Painter은 이제 Vulkan 그래픽 API를 뷰포트에서 렌더링 및 텍스처 계산에 사용합니다. 이 스위치는 응용 프로그램의 일반적인 성능을 개선해야 합니다. 또한 향후 새로운 기능을 보다 쉽게 통합할 수 있습니다.
* <b>Vulkan을 통해 굽기 위한 GPU 광선 추적</b>\
  DirectX 광선 추적(DRX) 및 Optix는 당사의 베이커에서 Vulkan 그래픽 API를 통해 광선 추적을 선호합니다. 이 변경은 이제 GPU 기반 광선 추적을 Linux 운영 체제뿐만 아니라 AMD GPU에서도 사용할 수 있음을 의미합니다.\
...
* <b>단일 큰 삼각형의 성능 개선</b>\
  지금까지 Painter은 매우 큰 삼각형 및/또는 긴 삼각형을 가진 매우 낮은 폴리 메쉬 또는 메쉬에서 고군분투할 수 있었습니다. 더 이상 그렇지 않습니다. 하나의 쿼드 메시를 사용한 작업, 예를 들어 타일링 텍스처를 만드는 작업은 더 이상 문제가 되어서는 안 됩니다.
* <b>기본 브러시 모양 개선</b>\
...
출시일: <b>2025/11/18</b>\
요약: <b>이 업데이트는 주요 릴리스이며 전용 새 콘텐츠가 포함된 새 리본 도구, 칠 레이어에 대한 대칭 지원, 변위에 대한 물리적 크기 매개 변수, 업데이트된 베이커를 통한 향상된 성능, Windows 및 Linux에 대한 전체 Vulkan 지원 및 기타 개선 사항이 포함되어 있습니다.</b>
...
* [리본] 페인트/지우개/손가락 패스 도구의 속성 창에서 패스/질감 미리 보기 제거
* [리본] 알파 및 일부 채널이 자체 겹칠 때 혼합 모드를 추가합니다
* 칠 대칭
* [칠] 칠 레이어 및 효과의 대칭 지원 추가
* [칠]&#x200B;[UI] 칠 레이어 및 효과의 속성 창에 대칭 설정 노출
* [Fill] 뷰포트 메뉴 및 속성 창에서 대칭 설정 UI 다시 작업
* [칠] 뒤틀기 모드에서 투영할 때 표준 텍스처의 방향을 올바르게
* 물리적 크기 변위
