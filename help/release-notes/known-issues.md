...
이 페이지에는 Substance 3D Painter v12.1.0에 있는 알려진 모든 활성 문제가 나열됩니다.
...
* `[Engine]` UV 패딩 &quot;3D Space Neighbor&quot; 모드가 얇은 삼각형에서 제대로 작동하지 않습니다.
* `[Engine]` 기준점 결과가 마스크와 색상 채널 간에 렌더링되지 않음
...
* `[Shelf]` `[Substance]` 사용자 데이터가 셸프 축소판 생성을 고려하지 않았습니다.
...
* 페인트 기울이기에서 `[Paint Skew]`개의 선택된 도구가 페인팅 모드로 전환된 후 선택된 상태로 있음
* 도구를 변경한 후 `[Color Picker]` 선택기가 열려 있음
* 내보낼 때 텍스처를 4k에서 8k로 업스케일할 때 `[UV Padding]`개 아티팩트
* `[Baking Common Settings]` 케이지 거리 설정이 케이지 와이어프레임 및 셰이더 시각화를 업데이트하지 않음
* `[Send to Photoshop]` 레이어의 마스크를 내보내지 못했습니다.
* 페인팅 및 실행 취소 시 `[Skew Baking]` 기울이기 수정이 중단됨
* 투영 도구에 의해 `[Projection Tool]` 뷰포트 상호 작용이 차단됨
...
* 일부 프로젝트를 열고 저장하는 데 평소보다 시간이 오래 소요될 수 있습니다.
...
* `[Crash]` 마스크 보기 모드에서 채널을 삭제하면 충돌이 발생합니다
* `[Crash]` 일부 Substance의 경우 렌더링 시 충돌이 발생할 수 있습니다
* `[Crash]` 굽기 모드에서 메시를 다시 가져옵니다.
* `[Crash]` 여러 메시를 다시 로드하면 충돌이 발생할 수 있습니다
