---
helpx_url: "https://helpx.adobe.com/kr/substance-3d-painter/release-notes/version-9-1.html"
breadcrumb-title: ''
...
<b>Substance 3D Painter 9.1</b>에는 [패스] 도구에 대한 접선 컨트롤, SVG 파일 형식 지원, 드래그 앤 드롭으로 리소스를 가져오고 적용하는 기능, 뷰포트의 투명도 지원이 추가되었습니다.
...
* <b>조작기를 통해 패스 지점 편집</b>
  ...
  개체 표면의 스니딩점만으로는 충분하지 않은 경우도 있습니다. 조작기를 사용하여 서피스 너머로 점을 이동할 수 있습니다. 메쉬를 다시 가져온 후 서피스에서 너무 멀리 떨어져 있는 경우처럼 여러 포인트를 한 번에 이동하는 것이 매우 도움이 될 수 있습니다.

  ![경로 접선 변환 조작기](../assets/path_tangents_translate.gif)
* <b>개별적으로 패스 표시 켜기/끄기</b>

  이제 전용 뷰포트 패널을 통해 패스별로 패스 가시성을 변경할 수 있습니다. 패스를 비활성화하면 패스를 삭제하지 않고도 최종 텍스처에서 기여도가 제거됩니다.
...
* <b>새 ASM 셰이더 설정</b>
  ...
  * <b>투명도</b>: 이 새로운 속성을 사용하면 유리처럼 표면을 만들 수 있어 Specular 반사를 유지하면서 모양을 투명하게 만들 수 있습니다. 이를 사용하려면 프로젝트에 Translucency 채널을 추가하고 <b>Interior</b> 그룹에서 <b>Translucency</b> 매개 변수를 사용하도록 설정하십시오.
...
* <b>Adobe 표준 자료를 데모할 새 샘플 프로젝트</b>
...
* <b>투명도 채널은 이제 기본적으로 검정색으로 설정됩니다</b>

  새 셰이더 속성을 보다 쉽게 사용하고 뷰포트에서 예기치 않은 결과가 나타나지 않도록 하기 위해 채널 반투명도의 기본 색상이 흰색 대신 검정색으로 변경되었습니다.

  프로젝트에서 이 채널을 이미 사용 중이었던 경우에는 레이어 스택 맨 아래에 칠 레이어를 추가하고 채널 값을 흰색으로 설정하면 이전 비헤이비어를 만들 수 있습니다. 셰이더 매개 변수에서 <b>투명도를 분산 마스크로 사용</b> 설정을 활성화하여 하위 표면 분산 결과에 채널 기여도를 다시 적용할 수 있습니다.

### 벡터 그래픽 파일에 대한 새로운 지원(SVG)
...
* <b>다양한 유형의 리소스 끌어다 놓기</b>
  ...
  이제 리소스 유형을 뷰포트 또는 레이어 스택으로 바로 드래그하여 놓을 수 있습니다. 이제 다음 유형의 리소스를 거의 모든 위치에서 드래그 앤 드롭할 수 있습니다.
...
* <b>뷰포트에 드롭하여 새 레이어 만들기</b>
  ...
  리소스를 드래그 앤 드롭하여 뷰포트에 새 레이어를 만들 수도 있습니다. 리소스 종류에 따라 결과가 달라질 수 있습니다. 필터를 사용하면 통과 모드에서 페인트 레이어가 만들어지고, 스마트 마스크를 사용하면 새 마스크가 있는 칠 레이어가 만들어집니다.

  ![스마트 마스크 드래그 앤 드롭](../assets/v91_drop_smart_mask.gif)
...
* <b>고급 동작에 키보드 보조키 사용</b>
  ...
  * <b>레이어 스택</b>에 드롭하는 동안 <b>ALT</b>: 레이어 축소판 위로 드롭하는 경우에만 적용됩니다. ALT 키를 누르면 이전의 모든 효과가 제거됩니다. 이렇게 하면 다른 리소스, 특히 스마트 마스크를 수동으로 먼저 제거할 필요 없이 빠르게 사용할 수 있습니다.
...
* <b>16비트 이미지의 손실 없는 압축</b>
  ...
  이 변경은 일반적으로 프로젝트 파일이 디스크에 매우 많을 수 있는 이유 중 하나인 <b>베이크 텍스처</b>를 대상으로 합니다. 평균적으로 프로젝트가 <b>디스크에서 크기가 30%에서 50%로 감소</b>되는 것을 보았습니다.
...
* <b>UV 세트 채우기 투영 모드로 새 UV 설정</b>
  ...
  칠 레이어/효과에 대한 새 투영 모드가 <b>UV 세트를 UV 세트 투영</b>으로 추가되었습니다. 이 효과는 프로젝트 내의 메쉬에 사용할 수 있는 다양한 UV를 기반으로 텍스처를 투영하는 데 사용할 수 있습니다. 이는 외부 도구에 의존할 필요 없이 보다 진보된 텍스처 전송을 수행하는 데 사용될 수 있다.
...
* <b>새로운 Python API 개선 사항</b>
  ...
  * 이제 프로젝트 내보내기 텍스처 경로를 API에서도 검색할 수 있습니다.
* <b>After Effects(베타)로 새 보내기</b>

  메시와 해당 텍스처를 After Effects으로 내보내는 새로운 보내기 동작을 사용할 수 있으므로 시각적 효과를 반복하기가 편리해집니다. 이 기능을 사용하려면 After Effects 버전 24.1 Beta 이상에 대한 액세스 권한이 필요합니다.
