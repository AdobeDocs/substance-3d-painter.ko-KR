...
1. ZBrush에서 **Painter으로 보내기**&#x200B;는 **텍스처 > Substance 브리지** 내에서 사용할 수 있습니다.
...
| PolyPaint 보내기 | [폴리 페인트]를 텍스처 맵으로 변환하고 Substance에서 칠 레이어로 적용하여 그 위에 페인트하고 혼합할 수 있습니다. |
| 스무드 노멀스 | 내보낼 때 탄젠트 법선을 매끄럽게 하여 Substance에서 면처리된 메시가 매끄럽게 나타나도록 하여 게임 엔진이 이를 렌더링하는 방식과 일치합니다. 도형의 실제 면을 보려면 끕니다. |
| 지도 자동 굽기 | 모델이 도달한 후 Substance의 베이킹 알고리즘을 자동으로 실행하여 높음/낮음 메시 비교에서 표준 맵, 주변 오클루전, 곡률 및 기타 세부 맵을 생성합니다. |
| 강제 UV 자동 줄 바꿈 | 도착하는 모든 하위 도구에 대해 Substance의 UV 감싸기 해제 알고리즘을 트리거합니다. UV가 이미 좋은 경우 덮어쓰므로 끄십시오. |
