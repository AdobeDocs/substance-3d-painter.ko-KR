...
색상 관리는 색상을 처리하고 변환하는 것입니다. 리소스 가져오기에서 화면에 색상 표시, 마지막으로 텍스처 내보내기에 이르기까지 다양한 작업을 수행할 수 있습니다. 색상 보정은 여러 응용 프로그램에서 동일한 모양을 보장하기 위해 중요합니다.

응용 프로그램 색상 관리는 [OpenColorIO](https://opencolorio.org/)&#x200B;(약어용 OCIO) 버전 2의 통합을 통해 처리됩니다. OCIO은 색상을 변환하고 표시하는 필름 및 애니메이션의 표준입니다. 색상 관리를 활성화하려면 새 프로젝트를 만들거나 기존 프로젝트를 열고 전용 설정을 활성화하기만 하면 됩니다.
...
* [Adobe ACE을 사용한 색상 관리 - ICC](color-management-with-adobe-ace-icc.md)
* [OpenColorIO을 사용하여 색상 관리](color-management-with-opencolorio.md)
...
| **디스플레이 변환** | 작업 영역의 색상 표시를 모니터에서 선형 색상으로 변환하여 사람의 눈으로 볼 수 있는 색상을 표시합니다. 변환 표시에는 제한된 범위의 색상을 화면에 맞게 압축하는 경우가 많습니다. |
| **구성** | OCIO 구성 파일. 작업 영역, 색상 공간 목록 및 디스플레이 목록을 정의합니다. |
| **ACE** | ACE은 Academy Color Encoding System을 의미하며 디지털 이미지 파일을 교환하는 많은 애플리케이션의 표준입니다. 이 표준의 두 버전은 기본적으로 응용 프로그램 내에 포함되어 있습니다. |
...
2D 및 3D 뷰포트 모두 색상 관리되며 상단 뷰포트에서 사용할 수 있는 전용 설정 을 통해 의 어떤 디스플레이를 사용할 것인지 제어할 수 있습니다.

![](../../assets/viewport-cm.png)

* **왼쪽 버튼**: 좌측뷰포트 표시 활성화/비활성화 비활성화된 경우 뷰포트에 색상이 원시/패스스루로 표시됩니다. 이 버튼은 기본적으로 활성화되어 있습니다.
* **오른쪽 드롭다운**: 어떤 디스플레이를 사용하여 화면에 색상을 변환할지 지정합니다. 기본값은 OCIO 구성을 기준으로 합니다. 이 설정은 모니터에 종속될 수 있으므로 프로젝트와 함께 저장되지 않습니다.
...
[텍스처 내보내기](../../export/export.md) 창 안에는 텍스처에 사용되는 색상 공간 **$colorSpace**&#x200B;을(를) 파일 이름에 추가하는 데 사용할 수 있는 키워드가 있습니다.
