---
helpx_url: "https://helpx.adobe.com/kr/substance-3d-painter/painting/presets/creating-particles-presets.html"
breadcrumb-title: ''
description: Substance 3D Painter에서 입자 브러시 사전 설정을 만들어 동적 텍스처 페인팅 효과를 워크플로우에 추가하는 방법을 알아봅니다.
helpx_creative_field: ""
helpx_description: Painter > Painting > Presets > Creating particles presets
helpx_experience_level: ""
helpx_learn_topic: ""
helpx_tags: ""
title: 파티클 사전 설정 만들기
user-guide-description: ''
user-guide-title: ''
source-git-commit: 9f20406f682e0e6a2e9a423e81c5ecfc7430ecfd
workflow-type: tm+mt
source-wordcount: '128'
ht-degree: 0%

---


# 파티클 사전 설정 만들기

입자를 페인팅할 수 있는 입자 시스템은 **.pkfx** 파일을 사용하는 **Popkfx**&#x200B;을(를) 사용하여 비헤이비어를 정의합니다.

파티클 브러시는 사전 설정 내에 함께 저장되는 **이미터** 및 **수신기**&#x200B;의 두 부분으로 구성됩니다. 이 응용 프로그램은 Emitter 를 로드하고 페인팅할 때 Receiver 를 방출하도록 충돌 이벤트를 재정의합니다.

자세한 내용은 전용 페이지 를 참조하십시오.

* [입자 편집기 설치](https://helpx.adobe.com/kr/substance-3d/unlisted/documentation/spdoc/installation-of-the-editor-67403780.html)
* [입자 편집기 개요](https://helpx.adobe.com/kr/substance-3d/unlisted/documentation/spdoc/overview-of-popcornfx-particle-editor-67796996.html)
* [새 입자 스크립트 만들기](creating-a-new-particle-script.md)
