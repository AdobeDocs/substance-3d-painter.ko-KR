...
breadcrumb-title: ''
description: Substance 3D Painter에서 직접 높이 맵을 페인트 하여 변위 및 표면 높이 효과를 만드는 방법을 살펴보세요.
...
* 높은 폴리 메쉬로부터 베이킹되는 노멀 맵이 낮은 폴리 메쉬 상에 로딩된다.
* Heightmap 채널에 추가 세부 정보를 페인트 할 수 있습니다.
* 페인트로 지정한 Height은 모든 레이어를 통해 합성되고 실시간으로 노멀 맵으로 변환되며, 최종적으로 상위 폴리 메쉬에서 표준으로 혼합됩니다.

Height 페인팅만 하면 나머지 모든 작업은 자동으로 수행됩니다.

### Height HDR 형식

Height 채널은 **HDR** 색상 형식을 사용하므로 채도가 0에서 255 사이로 올라가는 일반적인 높이 맵과는 달리 밝기의 한계에 도달하지 않고도 양과 음의 값을 페인트 할 수 있습니다.

* Height에서 비트맵 또는 Substance로 페인팅하면 해당 소스가 원래 [0,255] 범위에서 [-1,1] 범위로 다시 매핑됩니다.

중간 회색은 0으로 다시 매핑됩니다. 따라서 127 미만의 값은 높이 맵에서 **빼기**&#x200B;되고, 127 이상의 값은 높이 맵에 설정된 기본 혼합 모드 **선형 닷지(추가)**&#x200B;를 사용할 때 이 값에 **추가**&#x200B;됩니다.

* 일반 색상으로 페인트할 때 -1과 1 사이의 값을 직접 선택할 수 있습니다.

### Height 시각화

[단독] 모드에서 높이 맵을 시각화할 때 기본 미리 보기에는 양수 값만 표시되며 음수 값에는 강한 검정 채도가 표시됩니다.
