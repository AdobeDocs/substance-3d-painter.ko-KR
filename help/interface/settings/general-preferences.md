...
| 설정 | 설명 |
| --- | --- |
| **기본 재질 UV 비율** | 재질을 적용할 때 레이어 스택에서 칠 레이어 및 칠 효과의 기본 나란히 놓기/반복 값을 정의합니다. |
| **단순화된 축소판 사용** | 활성화되면 레이어 스택은 썸네일을 계산하는 대신 아이콘만 표시합니다. 아이콘을 사용하면 성능이 향상됩니다. UV 타일 워크플로를 사용하는 프로젝트에는 항상 아이콘이 표시되므로 이 설정은 적용되지 않습니다. |
...
| **GPU 광선 추적 사용** | 활성화되면 베이커는 CPU 대신 광선 추적 수행을 위해 GPU를 사용하려고 합니다. 이 기능을 통해 제빵사는 전반적으로 더 빠른 작업을 수행할 수 있습니다.호환되는 하드웨어에서만 활성화할 수 있습니다. 자세한 내용은 [시스템 요구 사항](../../getting-started/system-requirements.md)을 참조하세요. |
