...
이 섹션에서는 추가 리소스 폴더에 대한 사용자 정의 경로 및 기본적으로 저장할 위치 리소스를 지정할 수 있습니다.

## 기본 경로

기본적으로 두 개의 패스는 미리 정의됩니다.

| 이름 | 위치 |
| --- | --- |
| **내 에셋** | 이 경로는 현재 사용자 프로필의 문서 폴더에 있습니다. 여기서 사전 설정과 같은 리소스는 기본적으로 응용 프로그램 내에서 생성됩니다(이전 버전에서는 &quot;shelf&quot;로 이름 지정됨). |
| **스타터\_에셋** | 이 경로는 애플리케이션의 설치 폴더 내에 있습니다. 여기에는 기본 리소스가 포함되어 있습니다. (이전 버전에서는 이름이 &quot;알레고리학&quot; 또는 &quot;substance&quot;임) |

**기본값** 라디오 버튼은 새 내용(예: 브러시 사전 설정, 재질 사전 설정 또는 스마트 재질)을 저장할 경로를 정의하는 데 사용됩니다.

## 새 패스 추가

>[!NOTE]
>
> 현재 열려 있는 프로젝트가 없는 경우에만 경로를 만들거나 수정할 수 있습니다.
