...
지오메트리 마스크는 연관된 텍스처 세트의 3D 모델 지오메트리를 기반으로 레이어를 마스킹할 수 있는 레이어의 보조 마스크입니다. 메시 이름이나 UV 타일로 마스킹할 수 있습니다.

## 개요

지오메트리 마스크는 레이어가 포함/제외 목록을 통해 3D 모델의 어느 부분에 적용되어야 하는지 지정하는 방식으로 작동합니다.

[모양] 마스크는 3D 모델 모양의 큰 부분을 빠르게 버릴 수 있는 유용한 도구입니다. 다음과 같이 페인트 마스크에 여러 가지 이점을 제공합니다.

* 일반적으로 뷰포트 선택 모드를 사용하여 설정하고 사용하는 것이 더 빠릅니다.
* 텍스처를 생성할 때 지오메트리를 완전히 버릴 수 있어 더 나은 성능을 제공합니다.
* 비파괴적이며 다시 가져온 후 3D 모델이 변경되면 업데이트됩니다.
* 마스크된 지오메트리 아래에 있는 지오메트리를 페인트할 수 있어 숨겨진 부분을 페인트할 수 있습니다.
...
지정된 레이어의 지오메트리 마스크를 수정하려면 전용 아이콘을 클릭하면 됩니다. 편집 모드를 종료하려면 콘텐츠 또는 페인트 마스크와 같은 레이어의 다른 부분을 클릭하면 됩니다.
...
| 유형 | 설명 |
| --- | --- |
| **UV 타일** | 마스크는 포함할 UV 타일(UDIM) 번호를 지정하여 수행됩니다. 이 방법은 계산에서 텍스처를 완전히 버릴 수 있는 가장 강력한 방법입니다. |
...
아이콘을 마우스 오른쪽 버튼으로 클릭하면 레이어 스택에서 바로 지오메트리 마스크 상태를 빠르게 수정할 수 있습니다.
...
형상의 일부가 제외된 경우 뷰포트에서 숨길 수 있습니다. 그러면 이전에 아래에 있고 액세스할 수 없었던 도형을 페인트할 수 있습니다.
...
| 시각적 | 설명 |
| --- | --- |
| <div><img class="" data-preserve-html="true" id="root_content_flex_items_position_position-par_dx_table1_row-r1-column-c0_dynamic_grid_items_grid-cell_position-par_image" src="../../assets/no-geo-excluded.jpg" width="420px"/></div> | 지오메트리 마스크에서 제외된 지오메트리가 없습니다. 흰색 브러시 획이 수행된 페인트 레이어가 모든 지오메트리를 충돌합니다.**제외된 도형 숨기기** 단추를 사용할 수 없습니다. |
